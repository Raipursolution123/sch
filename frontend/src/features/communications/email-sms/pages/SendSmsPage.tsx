  ...
  .superRefine((values, ctx) => {
    if (values.audience === 'individual' && !(values.user_list || '').trim()) {
      ctx.addIssue({
        ...
        path: ['user_list'],
        message: 'Enter at least one mobile number.',
      });
    }
...
export function SendSmsPage() {
  ...
  const sendMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success('SMS composed and queued successfully');
      reset({
        title: '',
        message: '',
        audience: 'group',
    ...
    onError: () => {
      toast.error('Failed to send/schedule SMS');
    },
  });
  ...
  const templateOptions = useMemo(
    () => [
      { value: '', label: 'Select SMS template (optional)' },
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Send SMS"
        description="Compose and send SMS messages to classes, groups, or individual recipients."
      />
      ...
      >
        ...
        {audience === 'individual' && (
          <FormTextareaField
            control={control}
            name="user_list"
            label="Individual Mobile Numbers"
        ...
        <div className="flex justify-end pt-2">
          <PermissionButton
            permission="communicate.sms.send"
            type="submit"
