...
interface JournalEntryUpdateDialogProps {
  open: boolean;
...
export const JournalEntryUpdateDialog = ({
  open,
  onOpenChange,
  entry,
}: JournalEntryUpdateDialogProps) => {
  ...
  useEffect(() => {
    if (open && entry) {
      setDate(entry.date || '');
      setEntryTypeId(entry.entrytype_id ? String(entry.entrytype_id) : '');
      setNumber(entry.number ? String(entry.number) : '');
      ...
      if (entry.items && entry.items.length > 0) {
        setItems(
          entry.items.map((item) => ({
            ...
            ledger_id: String(item.ledger_id),
            amount: String(item.amount),
            dc: String(item.dc).toUpperCase() === 'C' ? 'C' : 'D',
