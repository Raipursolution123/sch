...
import { useExpenseHeads, useExpenseList } from '@hooks/useIncomeExpense';
...
export function SearchExpensePage() {
  ...
  const headNameMap = useMemo(() => {
    return new Map(heads.map((h) => [h.id, h.exp_category || `#${h.id}`]));
  }, [heads]);

  // Client-side date range filtering
  const filteredData = useMemo(() => {
    if (!submitted) return [];
    return rawExpense.filter((item) => {
      if (!item.date) return false;
      const d = item.date;
      return d >= fromDate && d <= toDate;
    });
  }, [rawExpense, fromDate, toDate, submitted]);
  ...
  const handleExportCsv = () => {
    ...
    exportToCsv(
      `expense-report-${fromDate}-to-${toDate}`,
      ['Name', 'Head', 'Invoice No', 'Date', 'Amount', 'Note'],
      filteredData.map((row) => [
        row.name || '—',
        row.exp_head_id ? headNameMap.get(row.exp_head_id) || `#${row.exp_head_id}` : '—',
        row.invoice_no || '—',
  ...
  return (
    <ModuleReportPack
      ...
      description="Search expense records by date range and query parameters."
      printTitle={`Expense Search Report (${fromDate} to ${toDate})`}
      onPrint={printReport}
      ...
      loadingMessage="Searching expense..."
