...
export function ZoomClassReportPage() {
  ...
  const handleSearch = async (e?: React.FormEvent) => {
    ...
    try {
      setLoading(true);
      const data = await zoomService.getClassReport({
  ...
  const handleViewViewers = async (c: Conference) => {
    try {
      setViewersOpen(true);
      setViewersLoading(true);
      setActiveClassName(c.title);
      const data = await zoomService.getClassViewers(c.id, {
        class_id: Number(selectedClass),
        section_id: Number(selectedSection),
      });
      setViewers(data || []);
  ...
  return (
    <ModuleListPack
      title="Class Report (Zoom)"
      description="View attendance and viewer statistics for live online Zoom classes."
      isLoading={loading}
...
export default ZoomClassReportPage;
