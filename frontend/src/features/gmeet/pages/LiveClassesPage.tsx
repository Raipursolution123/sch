...
import { gmeetService, type Gmeet } from '@services/api/gmeet.service';
...
export function LiveClassesPage() {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const data = await gmeetService.listClasses();
      setClasses(data || []);
      const settings = await gmeetService.getSettings();
      setUseApi(settings?.use_api === 1);
    } catch (error) {
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      await gmeetService.deleteClass(id);
  ...
  const handleJoin = async (c: Gmeet) => {
    try {
      window.open(c.url, '_blank');
      await gmeetService.join(c.id);
      fetchData();
    } catch (error) {
      console.error(error);
    }
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload = {
        ...
        type: useApi ? 'google_api' : 'manual',
        class_sections: form.class_sections,
        session_id: activeSession,
      };
      await gmeetService.createClass(payload);
  ...
  return (
    <ModuleListPack
      title="Live Classes"
      description="Create and join online Google Meet classes for students."
      isLoading={loading}
      onRetry={fetchData}
          ...
          <form onSubmit={handleSave} className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1.5 col-span-2">
                ...
                <Input
                  id="title"
                  placeholder="e.g. Mathematics - Algebra Intro"
                  value={form.title}
              ...
              {!useApi && (
                <div className="space-y-1.5 col-span-2">
                  <Label htmlFor="url">Manual GMeet URL</Label>
                  <Input
                    id="url"
