...
export function LiveMeetingPage() {
  const [meetings, setMeetings] = useState<Gmeet[]>([]);
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const data = await gmeetService.listMeetings();
      setMeetings(data || []);
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      await gmeetService.deleteMeeting(id);
  ...
  const handleJoin = async (m: Gmeet) => {
    try {
      window.open(m.url, '_blank');
      await gmeetService.join(m.id);
      fetchData();
    } catch (error) {
      console.error(error);
    }
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await gmeetService.createMeeting(payload);
  ...
  return (
    <ModuleListPack
      title="Live Meetings"
      description="Schedule and run staff meetings using Google Meet."
      isLoading={loading}
      onRetry={fetchData}
