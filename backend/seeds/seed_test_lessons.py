...
def seed_lessons():
    ...
    if created:
        ...
        print(f"Subject Group 'Math-Old' for Session 30 already exists with ID {old_group.id}")
    ...
    if created:
        ...
        print(f"Source SubjectGroupSubjects mapping already exists with ID {src_sgs.id}")
    ...
    if created:
        ...
        print(f"Source SubjectGroupClassSections mapping already exists with ID {src_sgcs.id}")

    # Clear existing lessons/topics for session 30
    existing_lessons = Lesson.objects.filter(session_id=30, subject_group_subject_id=src_sgs.id)
    ...
    print("Seeding complete! You can now test copying from Session 30 (2025-26) to Session 28 (2026-27).")
