...
class LedgerGroupsView(APIView):
    ...
    def get(self, request):
        ...
        return APIResponse.success(
            data=paginated_response.data, message="Ledger Groups fetched successfully"
        )
...
class LedgerGroupsDetailView(APIView):
    ...
    def get(self, request, pk):
        ...
        if not group:
            return APIResponse.error(
                message="Ledger Group not found", status_code=status.HTTP_404_NOT_FOUND
            )
        serializer = CycGroupsSerializer(group)
        return APIResponse.success(
            data=serializer.data, message="Ledger Group fetched successfully"
        )
    ...
    def put(self, request, pk):
        ...
        if not group:
            return APIResponse.error(
                message="Group not found", status_code=status.HTTP_404_NOT_FOUND
            )
        serializer = CycGroupsSerializer(group, data=request.data, partial=True)
        if serializer.is_valid():
            serializer.save()
            return APIResponse.success(
                data=serializer.data, message="Ledger Group updated successfully"
            )
        return APIResponse.error(
            message="Validation Error",
            details=serializer.errors,
