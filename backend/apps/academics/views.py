...
def _validate_session_format(session_name):
    ...
    if not match:
        return (
            False,
            "Session must be in format YYYY-YYYY or YYYY-YY (e.g., 2026-2027).",
        )
...
from .models import ClassSections, Classes, Sections, Sessions, Subjects
...
class ClassSectionsListView(APIView):
    ...
    def get(self, request):
            ...
            mappings_data.append(
                {
                    ...
                    "updated_at": (
                        m.updated_at.strftime("%Y-%m-%d") if m.updated_at else None
                    ),
                }
            )
        ...
        return Response(
            {
                "status": "success",
                "data": {"class_sections": mappings_data},
    ...
    def post(self, request):
        ...
        is_admin = getattr(request.user, "is_superadmin", False) or (
            user_role
            and str(user_role).strip().lower() in ["super admin", "admin", "superadmin"]
        )
        if not is_admin:
            return Response(
                {
                    "status": "error",
                    "message": "Access denied. Only Super Admins or Admins can create mappings.",
                },
        ...
        if not class_id or not section_id:
            return Response(
                {
                    "status": "error",
                    "message": "Both class_id and section_id are required.",
                },
        ...
        try:
            ...
            return Response(
                {
                    "status": "error",
                    "message": "class_id and section_id must be integers.",
                },
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        if ClassSections.objects.filter(
        ...
        try:
            ...
            logger.info(
                f"Class-Section mapping created by user {(request.user.username if request.user.is_authenticated else 'Unknown')}."
            )
            return Response(
                {
                    "status": "success",
                    "data": {
                        ...
                        "created_at": new_mapping.created_at.strftime(
                            "%Y-%m-%d %H:%M:%S"
                        ),
                    },
    ...
    def post(self, request):
        ...
        if not is_admin:
            return Response(
                {
                    "status": "error",
                    "message": "Access denied. Only Super Admins or Admins can assign sections to classes.",
                },
                status=status.HTTP_403_FORBIDDEN,
            )
        ...
        if not class_id or section_ids is None:
            return Response(
                {
                    "status": "error",
                    "message": "Please provide class_id and section_ids list.",
                },
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        if not isinstance(section_ids, list):
            return Response(
                {"status": "error", "message": "section_ids must be a list."},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        try:
            ...
            return Response(
                {
                    "status": "error",
                    "message": "class_id and section_ids must contain valid integers.",
                },
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        try:
                ...
                for sec_id, m in mapping_dict.items():
                    if sec_id not in section_ids:
                        ...
                        if active_student_sessions:
                            ...
                            raise ValueError(
                                f"Cannot remove section '{sec_obj_remove.section}' from class '{class_obj.class_field}' because active students are assigned to this specific mapping."
                            )
            ...
            logger.info(
                f"ClassSections updated for class '{class_obj.class_field}' by user {(request.user.username if request.user.is_authenticated else 'Unknown')}."
            )
            return Response(
                {
                    "status": "success",
                    "message": f"Sections successfully assigned/updated for class '{class_obj.class_field}'.",
                },
                status=status.HTTP_200_OK,
            )
            ...
            return Response(
                {"status": "error", "message": f"Database transaction error: {str(e)}"},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
...
class ClassAssignedSectionsView(APIView):
    ...
    def get(self, request, class_id):
        ...
        current_page = (
            paginated_mappings if paginated_mappings is not None else mappings
        )
        section_ids = [m.section_id for m in current_page if m.section_id]
        sections_dict = {
            s.id: s.section for s in Sections.objects.filter(id__in=section_ids)
        ...
        if paginated_mappings is not None:
            return paginator.get_paginated_response(
                {"class": class_obj.class_field, "assigned_sections": sections_data}
            )
...
class ClassSectionsDetailView(APIView):
    ...
    def get(self, request, pk):
        ...
        try:
            ...
            return Response(
                {"status": "error", "message": "Class-Section mapping not found."},
                status=status.HTTP_404_NOT_FOUND,
            )

        class_obj = (
            Classes.objects.filter(id=mapping.class_id).first()
            if mapping.class_id
            else None
        )
        section_obj = (
            Sections.objects.filter(id=mapping.section_id).first()
        ...
        return Response(
            {
                ...
                "data": {
                    ...
                    "updated_at": (
                        mapping.updated_at.strftime("%Y-%m-%d")
    ...
    def put(self, request, pk):
        ...
        if not is_admin:
            return Response(
                {
                    "status": "error",
                    "message": "Access denied. Only Super Admins or Admins can modify mappings.",
                },
                status=status.HTTP_403_FORBIDDEN,
            )
        ...
        if is_active is not None:
            if str(is_active).lower() not in ["yes", "no"]:
                return Response(
                    {"status": "error", "message": "is_active must be 'yes' or 'no'."},
                    status=status.HTTP_400_BAD_REQUEST,
                )

            # Deactivation dependency checks
            if (
                str(is_active).lower() == "no"
                and mapping.class_id
                ...
                if active_student_sessions:
                    return Response(
                        {
                            "status": "error",
                            "message": f"Cannot deactivate mapping. Active students are currently assigned to this mapping.",
                        },
                        status=status.HTTP_400_BAD_REQUEST,
                    )
        ...
        try:
            ...
            mapping.save()
            logger.info(
                f"Class-Section mapping ID {pk} updated by user {(request.user.username if request.user.is_authenticated else 'Unknown')}."
            )
            return Response(
                {
                    "status": "success",
    ...
    def delete(self, request, pk):
        ...
        if not is_admin:
            return Response(
                {
                    "status": "error",
                    "message": "Access denied. Only Super Admins or Admins can delete mappings.",
                },
                status=status.HTTP_403_FORBIDDEN,
            )
...
class SectionsListCreateView(APIView):
    ...
    def get(self, request):
        ...
        paginator = StandardResultsSetPagination()
        paginated_sections = paginator.paginate_queryset(
            sections_qs, request, view=self
        )
...
class SectionsDetailView(APIView):
    ...
    def get(self, request, pk):
        ...
        return APIResponse.success(
            data={
                ...
                "created_at": (
                    section_obj.created_at.strftime("%Y-%m-%d %H:%M:%S")
                    if section_obj.created_at
                    else None
                ),
                "updated_at": (
                    section_obj.updated_at.strftime("%Y-%m-%d")
                    if section_obj.updated_at
                    else None
                ),
            },
    ...
    def put(self, request, pk):
        ...
        if section_name is not None:
            ...
            if (
                Sections.objects.filter(section__iexact=section_name_clean)
                .exclude(pk=pk)
                .exists()
            ):
                return APIResponse.error(
        ...
        try:
            ...
            section_obj.save()
            logger.info(
                f"Section ID {pk} updated by user {(request.user.username if request.user.is_authenticated else 'Unknown')}."
            )
            return APIResponse.success(
                data={
                    "id": section_obj.id,
    ...
    def delete(self, request, pk):
        ...
        active_mappings = ClassSections.objects.filter(
            section_id=section_obj.id, is_active="yes"
        ).exists()
        if active_mappings:
            return APIResponse.error(
        ...
        try:
            ...
            section_obj.save()
            logger.info(
                f"Section ID {pk} deactivated by user {(request.user.username if request.user.is_authenticated else 'Unknown')}."
            )
...
class SessionsDetailView(APIView):
    ...
    def get(self, request, pk):
        ...
        return APIResponse.success(
            data={
                ...
                "created_at": (
                    session_obj.created_at.strftime("%Y-%m-%d %H:%M:%S")
                    if session_obj.created_at
                    else None
                ),
                "updated_at": (
                    session_obj.updated_at.strftime("%Y-%m-%d")
    ...
    def put(self, request, pk):
        ...
        if session_name is not None:
            ...
            if (
                Sessions.objects.filter(session__iexact=session_name_clean)
                .exclude(pk=pk)
                .exists()
            ):
                return APIResponse.error(
        ...
        try:
            ...
            session_obj.save()
            logger.info(
                f"Session ID {pk} updated by user {(request.user.username if request.user.is_authenticated else 'Unknown')}."
            )
            return APIResponse.success(
                data={
                    "id": session_obj.id,
    ...
    def delete(self, request, pk):
        ...
        try:
            ...
            logger.info(
                f"Session ID {session_id} deleted by user {(request.user.username if request.user.is_authenticated else 'Unknown')}."
            )
...
class SessionActivateView(APIView):
    ...
    def post(self, request, pk):
        ...
        try:
            ...
            logger.info(
                f"Session '{session_obj.session}' activated by user {(request.user.username if request.user.is_authenticated else 'Unknown')}."
            )
            return APIResponse.success(
                data={
                    "id": session_obj.id,
...
class ClassesListCreateView(APIView):
    ...
    def get(self, request):
        ...
        class_sections_qs = ClassSections.objects.filter(
            class_id__in=class_ids, is_active="yes"
        )
        section_ids_all = list(
            set(cs.section_id for cs in class_sections_qs if cs.section_id)
        )
        sections_dict = {
            s.id: s.section for s in Sections.objects.filter(id__in=section_ids_all)
        }
        ...
        for cs in class_sections_qs:
            ...
            if sec_name:
                class_sections_map[cs.class_id].append(
                    {"id": cs.section_id, "section_name": sec_name}
                )
            ...
            classes_data.append(
                {
                    ...
                    "is_hedu_program": c.is_hedu_program == "yes"
                    or c.is_hedu_program == "true"
                    or c.is_hedu_program == "True",
                    ...
                    "updated_at": (
                        c.updated_at.strftime("%Y-%m-%d") if c.updated_at else None
                    ),
                }
            )
    ...
    def post(self, request):
        ...
        try:
            ...
            return APIResponse.error(
                message="sort_order must be an integer.", status_code=400
            )

        is_hedu_program_raw = data.get("is_hedu_program", "")
        is_hedu_program = (
            "yes"
            if is_hedu_program_raw is True
            or str(is_hedu_program_raw).lower() in ["yes", "true", "1"]
            else "no"
        )
        ...
        if sections:
            try:
                ...
                valid_sections_count = Sections.objects.filter(
                    id__in=section_ids
                ).count()
        ...
        try:
            ...
            return APIResponse.success(
                data={
                    ...
                    "is_hedu_program": new_class.is_hedu_program == "yes"
                    or new_class.is_hedu_program == "true"
                    or new_class.is_hedu_program == "True",
                    "is_active": new_class.is_active,
                    "sections": [
                        {
                            "id": sid,
                            "section_name": Sections.objects.get(id=sid).section,
                        }
...
class ClassesDetailView(APIView):
    ...
    def get(self, request, pk):
        ...
        return APIResponse.success(
            data={
                ...
                "is_hedu_program": class_obj.is_hedu_program == "yes"
                or class_obj.is_hedu_program == "true"
                or class_obj.is_hedu_program == "True",
                ...
                "updated_at": (
                    class_obj.updated_at.strftime("%Y-%m-%d")
    ...
    def put(self, request, pk):
        ...
        if class_name is not None:
            ...
            if (
                Classes.objects.filter(class_field__iexact=class_name_clean)
                .exclude(pk=pk)
                .exists()
            ):
                return APIResponse.error(
        ...
        if is_hedu_program is not None:
            class_obj.is_hedu_program = (
                "yes"
                if is_hedu_program is True
                or str(is_hedu_program).lower() in ["yes", "true", "1"]
                else "no"
            )
        ...
        try:
            with transaction.atomic():
                ...
                if section_ids is not None:
                    existing_mappings = ClassSections.objects.filter(
                        class_id=class_obj.id
                    )
                    mapping_dict = {m.section_id: m for m in existing_mappings}
            ...
            logger.info(
                f"Class ID {pk} and its sections updated by user {(request.user.username if request.user.is_authenticated else 'Unknown')}."
            )
    ...
    def delete(self, request, pk):
        ...
        try:
            ...
            logger.info(
                f"Class ID {pk} deleted by user {(request.user.username if request.user.is_authenticated else 'Unknown')}."
            )
...
class SubjectsListCreateView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            logger.info(
                f"Subject '{new_subject.name}' created by user {(request.user.username if request.user.is_authenticated else 'Unknown')}."
            )
            return APIResponse.success(
                data={
                    "id": new_subject.id,
...
class SubjectsDetailView(APIView):
    ...
    def get(self, request, pk):
        ...
        if subject_obj is None:
            return APIResponse.error(
                message="Subject not found.", status_code=status.HTTP_404_NOT_FOUND
            )
        ...
        return APIResponse.success(
            data={
                ...
                "created_at": (
                    subject_obj.created_at.strftime("%Y-%m-%d %H:%M:%S")
                    if subject_obj.created_at
                    else None
                ),
                "updated_at": (
                    subject_obj.updated_at.strftime("%Y-%m-%d")
                    if subject_obj.updated_at
                    else None
                ),
            },
    ...
    def put(self, request, pk):
        ...
        try:
            ...
            subject_obj.save()
            logger.info(
                f"Subject ID {pk} updated by user {(request.user.username if request.user.is_authenticated else 'Unknown')}."
            )
    ...
    def delete(self, request, pk):
        ...
        try:
            ...
            subject_obj.delete()
            logger.info(
                f"Subject '{subject_name}' (ID {pk}) deleted by user {(request.user.username if request.user.is_authenticated else 'Unknown')}."
            )
            return APIResponse.success(
