...
class FinancialReportView(APIView):
    ...
    def _get(self, request):
        ...
        # Revenue
        general_income = float(Income.objects.filter(
        ...
        fee_breakdown_query = OfflineFeesPayments.objects.filter(
            payment_date__range=[start_date, end_date]
        ).values('fee_groups_feetype_id').annotate(total=Sum('amount'))

        # Build mapping for fee types
        fee_group_ids = [item['fee_groups_feetype_id'] for item in fee_breakdown_query if item['fee_groups_feetype_id']]
        ...
        for dep in all_deposites:
            ...
            for payment in payment_list:
                ...
                if start_date <= p_date <= end_date:
                    ...
                    if dep.fee_groups_feetype_id and dep.fee_groups_feetype_id not in fee_group_ids:
                        fee_group_ids.append(dep.fee_groups_feetype_id)

        # Map fee group ids to feetype names
        fee_groups = FeeGroupsFeetype.objects.filter(id__in=fee_group_ids).values('id', 'feetype_id')
        ...
        fg_to_feetype_map = {fg['id']: feetype_map.get(fg['feetype_id'], 'Unknown Fee') for fg in fee_groups}
        ...
        for item in fee_breakdown_query:
            ...
            try:
                ...
                amount = 0.0
            fee_name = fg_to_feetype_map.get(fg_id, 'Uncategorized Fee') if fg_id else 'Uncategorized Fee'
            student_fees_breakdown[fee_name] = student_fees_breakdown.get(fee_name, 0.0) + amount
        ...
        # Expenditures
        try:
            general_expenses = float(Expenses.objects.filter(
                date__range=[start_date, end_date], 
                is_active="yes", 
                is_deleted="no"
            ).aggregate(total=Sum('amount'))['total'] or 0.0)
        ...
        payroll_query = StaffPayslip.objects.filter(
            ...
            Q(month__in=[m for m, y in months_years], year__in=[y for m, y in months_years])
        )
        for ps in payroll_query:
            if ps.staff_id and ps.month and ps.year:
                try:
                    payslip_map[(ps.staff_id, ps.month, ps.year)] = float(ps.net_salary if ps.net_salary is not None else 0.0)
                except (ValueError, TypeError):
        ...
        for staff in all_staff:
            s_name = f"{staff.name or ''} {staff.surname or ''}".strip() or f"Staff #{staff.id}"
            try:
            ...
            staff_payroll_breakdown[s_name] = staff_payroll_breakdown.get(s_name, 0.0) + staff_total_for_period
