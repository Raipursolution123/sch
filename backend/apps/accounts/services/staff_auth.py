...
from apps.accounts.services.legacy_password import hash_legacy_password, verify_staff_password
...
def authenticate_staff(username: str, password: str) -> User | None:
    ...
    staff = (
        Staff.objects.filter(email=username, is_active=1)
        .order_by("id")
        .first()
    )
...
def ensure_staff_user_bridge(staff: Staff) -> User:
    user = User.objects.filter(username=staff.email, role="staff", user_id=staff.id).first()
    if user:
