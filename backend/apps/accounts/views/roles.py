...
from apps.accounts.serializers.role import RolePermissionSerializer, RoleSerializer
...
class RolesListCreateView(APIView):
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            serializer.save(created_at=timezone.now(), is_active=1)
            return APIResponse.success(
...
class RoleDetailView(APIView):
    ...
    def get(self, request, pk):
        ...
        return APIResponse.success(
            data=serializer.data, message="Role details retrieved."
        )
    ...
    def put(self, request, pk):
        ...
        if serializer.is_valid():
            ...
            return APIResponse.success(
                data=serializer.data, message="Role updated successfully."
            )
        return APIResponse.error(
            message="Validation Error",
            details=serializer.errors,
    ...
    def get(self, request, role_id):
        ...
        for cat in categories:
            perm = RolePermission.objects.filter(
                role=role, permission_category=cat
            ).first()
            if not perm:
                perm = RolePermission(
                    role=role,
        ...
        return APIResponse.success(
            data=serializer.data, message="Role permissions retrieved."
        )
