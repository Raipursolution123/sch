    ...
    def get(self, request):
        ...
        if active_session:
            student_sessions = StudentSession.objects.filter(
                session_id=active_session.id, student_id__in=student_ids
            )
            ...
            data.append(
                {
                    ...
                    "section_name": (
                        sections_dict.get(section_id) if section_id else None
                    ),
                    "admission_date": safe_date_str(student.admission_date),
                    "created_at": safe_date_str(
                        student.created_at, "%Y-%m-%dT%H:%M:%SZ"
                    ),
                }
            )
        ...
        try:
            with transaction.atomic():
                student = Students.objects.create(
                    ...
                    gender=(
                        data.get("gender", "").strip() if data.get("gender") else None
                    ),
                    mobileno=(
                        data.get("mobileno", "").strip()
                    ...
                    pincode=(
                        data.get("pincode", "").strip() if data.get("pincode") else None
                    ),
                    religion=(
                        data.get("religion", "").strip()
                    ...
                    father_occupation=(
                        data.get("father_occupation", "").strip()
                        if data.get("father_occupation")
                        else None
                    ),
                    mother_name=(
                        data.get("mother_name", "").strip()
                        if data.get("mother_name")
                        else None
                    ),
                    mother_phone=(
                        data.get("mother_phone", "").strip()
                        if data.get("mother_phone")
                        else None
                    ),
                    mother_occupation=(
                        data.get("mother_occupation", "").strip()
                    ...
                    guardian_relation=(
                        data.get("guardian_relation", "").strip()
                        if data.get("guardian_relation")
                        else None
                    ),
                    guardian_phone=(
                        data.get("guardian_phone", "").strip()
                        if data.get("guardian_phone")
                        else None
                    ),
                    guardian_occupation=(
                        data.get("guardian_occupation", "").strip()
                        if data.get("guardian_occupation")
                        else ""
                    ),
                    guardian_address=(
                        data.get("guardian_address", "").strip()
                        if data.get("guardian_address")
                        else None
                    ),
                    guardian_email=(
                        data.get("guardian_email", "").strip()
                        if data.get("guardian_email")
                        else None
                    ),
                    guardian_is=data.get("guardian_is", "father").strip(),
                    current_address=(
                        data.get("current_address", "").strip()
                        if data.get("current_address")
                        else None
                    ),
                    permanent_address=(
                        data.get("permanent_address", "").strip()
                    ...
                    school_house_id=(
                        int(data.get("school_house_id"))
                        if data.get("school_house_id")
                        else None
                    ),
                    hostel_room_id=(
                        int(data.get("hostel_room_id"))
                        if data.get("hostel_room_id")
                        else 0
                    ),
                    room_bed_id=(
                        int(data.get("room_bed_id")) if data.get("room_bed_id") else 0
                    ),
                    adhar_no=(
                        data.get("adhar_no", "").strip()
                        if data.get("adhar_no")
                        else None
                    ),
                    samagra_id=(
                        data.get("samagra_id", "").strip()
                        if data.get("samagra_id")
                        else None
                    ),
                    bank_account_no=(
                        data.get("bank_account_no", "").strip()
                        if data.get("bank_account_no")
                        else None
                    ),
                    bank_name=(
                    ...
                    ifsc_code=(
                        data.get("ifsc_code", "").strip()
                        if data.get("ifsc_code")
                        else None
                    ),
                    previous_school=(
                        data.get("previous_school", "").strip()
                ...
                detail = _get_student_detail(
                    student,
                    class_id,
                    section_id,
                    school_class.class_field,
                    section.section,
                )
                return APIResponse.success(
                    data=detail, message="Student created successfully."
                )
        except Exception as e:
...
class StudentDetailView(APIView):
    ...
    def get(self, request, pk):
        ...
        detail = _get_student_detail(
            student, class_id, section_id, class_name, section_name
        )
    ...
    def put(self, request, pk):
        ...
        admission_no = (
            data.get("admission_no", "").strip() if "admission_no" in data else None
        )
        if (
            admission_no
            and Students.objects.exclude(id=pk)
            ...
            with transaction.atomic():
                ...
                if "admission_date" in data:
                    student.admission_date = (
                        data["admission_date"] if data["admission_date"] else None
                    )
                if "firstname" in data:
                    student.firstname = data["firstname"].strip()
                if "middlename" in data:
                    student.middlename = (
                        data["middlename"].strip() if data["middlename"] else None
                    )
                if "lastname" in data:
                    student.lastname = data["lastname"].strip()
                if "gender" in data:
                    student.gender = data["gender"]
                if "mobileno" in data:
                    student.mobileno = (
                        data["mobileno"].strip() if data["mobileno"] else None
                    )
                if "email" in data:
                    student.email = data["email"].strip() if data["email"] else None
                if "state" in data:
                    student.state = data["state"].strip() if data["state"] else None
                if "city" in data:
                    student.city = data["city"].strip() if data["city"] else None
                if "pincode" in data:
                    student.pincode = (
                        data["pincode"].strip() if data["pincode"] else None
                    )
                if "religion" in data:
                    student.religion = (
                        data["religion"].strip() if data["religion"] else None
                    )
                if "cast" in data:
                    student.cast = data["cast"].strip() if data["cast"] else None
                if "dob" in data:
                    student.dob = data["dob"] if data["dob"] else None
                if "is_active" in data:
                    student.is_active = data["is_active"]
                if "father_name" in data:
                    student.father_name = (
                        data["father_name"].strip() if data["father_name"] else None
                    )
                if "father_phone" in data:
                    student.father_phone = (
                        data["father_phone"].strip() if data["father_phone"] else None
                    )
                if "father_occupation" in data:
                    student.father_occupation = (
                        data["father_occupation"].strip()
                        if data["father_occupation"]
                        else None
                    )
                if "mother_name" in data:
                    student.mother_name = (
                        data["mother_name"].strip() if data["mother_name"] else None
                    )
                if "mother_phone" in data:
                    student.mother_phone = (
                        data["mother_phone"].strip() if data["mother_phone"] else None
                    )
                if "mother_occupation" in data:
                    student.mother_occupation = (
                        data["mother_occupation"].strip()
                        if data["mother_occupation"]
                        else None
                    )
                if "guardian_name" in data:
                    student.guardian_name = (
                        data["guardian_name"].strip() if data["guardian_name"] else None
                    )
                if "guardian_relation" in data:
                    student.guardian_relation = (
                        data["guardian_relation"].strip()
                        if data["guardian_relation"]
                        else None
                    )
                if "guardian_phone" in data:
                    student.guardian_phone = (
                        data["guardian_phone"].strip()
                        if data["guardian_phone"]
                        else None
                    )
                if "guardian_occupation" in data:
                    student.guardian_occupation = (
                        data["guardian_occupation"].strip()
                        if data["guardian_occupation"]
                        else ""
                    )
                if "guardian_address" in data:
                    student.guardian_address = (
                        data["guardian_address"].strip()
                        if data["guardian_address"]
                        else None
                    )
                if "guardian_email" in data:
                    student.guardian_email = (
                        data["guardian_email"].strip()
                        if data["guardian_email"]
                        else None
                    )
                if "guardian_is" in data:
                    student.guardian_is = (
                        data["guardian_is"].strip() if data["guardian_is"] else "father"
                    )
                if "current_address" in data:
                    student.current_address = (
                        data["current_address"].strip()
                        if data["current_address"]
                        else None
                    )
                if "permanent_address" in data:
                    student.permanent_address = (
                        data["permanent_address"].strip()
                ...
                if "blood_group" in data:
                    student.blood_group = (
                        data["blood_group"].strip() if data["blood_group"] else ""
                    )
                if "school_house_id" in data:
                    student.school_house_id = (
                        int(data["school_house_id"])
                        if data["school_house_id"]
                        else None
                    )
                if "hostel_room_id" in data:
                    student.hostel_room_id = (
                        int(data["hostel_room_id"]) if data["hostel_room_id"] else 0
                    )
                if "room_bed_id" in data:
                    student.room_bed_id = (
                        int(data["room_bed_id"]) if data["room_bed_id"] else 0
                    )
                if "adhar_no" in data:
                    student.adhar_no = (
                        data["adhar_no"].strip() if data["adhar_no"] else None
                    )
                if "samagra_id" in data:
                    student.samagra_id = (
                        data["samagra_id"].strip() if data["samagra_id"] else None
                    )
                if "bank_account_no" in data:
                    student.bank_account_no = (
                        data["bank_account_no"].strip()
                        if data["bank_account_no"]
                        else None
                    )
                if "bank_name" in data:
                    student.bank_name = (
                        data["bank_name"].strip() if data["bank_name"] else None
                    )
                if "ifsc_code" in data:
                    student.ifsc_code = (
                        data["ifsc_code"].strip() if data["ifsc_code"] else None
                    )
                if "previous_school" in data:
                    student.previous_school = (
                        data["previous_school"].strip()
                        if data["previous_school"]
                        else None
                    )
                if "height" in data:
                    student.height = data["height"].strip() if data["height"] else ""
                if "weight" in data:
                    student.weight = data["weight"].strip() if data["weight"] else ""
                if "measurement_date" in data:
                    student.measurement_date = (
                        data["measurement_date"] if data["measurement_date"] else None
                    )
                if "note" in data:
                    student.note = data["note"].strip() if data["note"] else None
                if "rte" in data:
                    student.rte = data["rte"]
                ...
                class_name = (
                    ...
                    if school_class
                        ...
                        Classes.objects.filter(id=ss.class_id).first().class_field
                        if ss and ss.class_id
                        else None
                    )
                )
                section_name = (
                    ...
                    if section
                        ...
                        Sections.objects.filter(id=ss.section_id).first().section
                        if ss and ss.section_id
                        else None
                    )
                )
                ...
                detail = _get_student_detail(
                    student,
                    ss.class_id if ss else class_id,
                    ss.section_id if ss else section_id,
                    class_name,
                    section_name,
                )
                return APIResponse.success(
                    data=detail, message="Student updated successfully."
                )
        except Exception as e:
...
class StudentFeesView(APIView):
    ...
    def get(self, request, pk):
        ...
        with connection.cursor() as cursor:
            ...
            print(
                "DEBUG: Retrieved payments count:", len(payments)
            )
