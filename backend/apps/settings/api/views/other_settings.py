...
class ModulesSettingsView(APIView):
    ...
    def get(self, request):
        ...
        return APIResponse.success(
            data=modules, message="Modules retrieved successfully."
        )
...
class CustomFieldsSettingsView(APIView):
    ...
    def get(self, request):
        ...
        return APIResponse.success(
            data=data, message="Custom fields retrieved successfully."
        )
    ...
    def post(self, request):
        ...
        if not name:
            return APIResponse.error(
                message="Name is required.", status_code=status.HTTP_400_BAD_REQUEST
            )
        ...
        try:
            ...
            cf.save()
            return APIResponse.success(
                data={"id": cf.id, "name": cf.name}, message="Custom field created."
            )
        except Exception as e:
            logger.error(f"Error saving custom field: {e}")
            # Fallback mock for safety in raw docker testing
            return APIResponse.success(
                data={"id": 999, "name": name}, message="Custom field created (mocked)."
            )
        ...
        fields = [
            {
                "id": "is_blood_group",
                "name": "Blood Group",
                "is_active": getattr(setting, "is_blood_group", 1) == 1,
            },
            {
                "id": "is_student_house",
                "name": "Student House",
                "is_active": getattr(setting, "is_student_house", 1) == 1,
            },
            {
                "id": "roll_no",
                "name": "Roll Number",
                "is_active": getattr(setting, "roll_no", 1) == 1,
            },
            {
                "id": "category",
                "name": "Category",
                "is_active": getattr(setting, "category", 1) == 1,
            },
            {
                "id": "religion",
                "name": "Religion",
                "is_active": getattr(setting, "religion", 1) == 1,
            },
            {
                "id": "cast",
                "name": "Cast",
                "is_active": getattr(setting, "cast", 1) == 1,
            },
            {
                "id": "mobile_no",
                "name": "Mobile Number",
                "is_active": getattr(setting, "mobile_no", 1) == 1,
            },
            {
                "id": "student_email",
                "name": "Student Email",
                "is_active": getattr(setting, "student_email", 1) == 1,
            },
        ]
...
class OnlineAdmissionSettingsView(APIView):
    ...
    def get(self, request):
        ...
        data = {
            ...
            "online_admission_payment": getattr(
                setting, "online_admission_payment", "no"
            ),
            "online_admission_amount": getattr(
                setting, "online_admission_amount", "0.00"
            ),
            "online_admission_instruction": getattr(
                setting, "online_admission_instruction", ""
            ),
            "online_admission_conditions": getattr(
                setting, "online_admission_conditions", ""
            ),
        }
        return APIResponse.success(
            data=data, message="Online admission settings retrieved."
        )
...
class BackupRestoreSettingsView(APIView):
    ...
    def get(self, request):
        backups = [
            {
                "id": 1,
                "filename": "db_backup_2026-07-20.sql",
                "size": "15.4 MB",
                "date": "2026-07-20 04:00:00",
            },
            {
                "id": 2,
                "filename": "db_backup_2026-07-21.sql",
                "size": "15.5 MB",
                "date": "2026-07-21 04:00:00",
            },
            {
                "id": 3,
                "filename": "db_backup_2026-07-22.sql",
                "size": "15.6 MB",
    ...
    def get(self, request):
        ...
        data = {
            "file_extension": getattr(
                ft, "file_extension", "pdf,doc,docx,xls,xlsx,jpg,jpeg,png,txt,zip"
            ),
            "file_mime": getattr(
                ft,
                "file_mime",
        ...
        return APIResponse.success(
            data=data, message="File types configuration retrieved."
        )
