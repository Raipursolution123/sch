...
class EmailSettingsView(APIView):
    ...
    def get(self, request):
        ...
        if not config:
            config = EmailConfig.objects.create(
                email_type="smtp", is_active="no", created_at=timezone.now()
            )
        serializer = EmailConfigSerializer(config)
        return APIResponse.success(
            data=serializer.data, message="Email configuration retrieved successfully"
        )
    ...
    def put(self, request):
        ...
        if serializer.is_valid():
            serializer.save()
            return APIResponse.success(
                data=serializer.data, message="Email configuration updated successfully"
            )
