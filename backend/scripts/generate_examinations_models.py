"""Generate examinations Django models from examinations_domain_inventory.json."""
...
OUT_DIR = Path(__file__).resolve().parent.parent / "apps" / "examinations" / "models"
...
def main():
    ...
    for table, class_name in sorted(generated, key=lambda x: x[0]):
        init_lines.append(
            f"from apps.examinations.models.{table} import {class_name}"
        )
    init_lines.append("")
    ...
    print(f"Generated {len(generated)} examination models in {OUT_DIR}")
